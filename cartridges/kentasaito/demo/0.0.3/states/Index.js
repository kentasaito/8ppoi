...
export class Index {
  ...
  static onFrame() {
    if (Input.pads[0].statuses.directions.left & 2 && this.player.x > 0) {
      this.player.x -= 4;
    }
    if (
      Input.pads[0].statuses.directions.right & 2 &&
      this.player.x < this.numColumn * 4 - 4
    ) {
      this.player.x += 4;
    }
    if (Input.pads[0].statuses.actions.down & 2 && !this.bulletTask) {
